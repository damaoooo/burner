...
In the SLURM allocation panel, `Sample / UI Refresh (ms)` defaults to `30`. This controls how often workers publish latest load metrics and how often the UI polls them. CSV sample files are decimated to about once per second to keep shared filesystem writes bounded.
