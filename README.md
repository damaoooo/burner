...
| `--tick` | Scheduler update interval in seconds. Defaults to `0.01`. |
...
The CPU and GPU patched backends also check their utilization control files at roughly `10ms` granularity, though the effective GPU response still depends on CUDA work-loop latency.
